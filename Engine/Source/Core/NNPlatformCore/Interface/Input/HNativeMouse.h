...
namespace NN::Core
{
	...
	class H_CORE_PLATFORM_API HMouseButtonEvent
	{
	public:
		enum class Type
		{
	...
	class H_CORE_PLATFORM_API HMouseContext
		:
...
#pragma prefast(suppress: 26495, "Reset() performs the initialization")
	...
	struct H_CORE_PLATFORM_API HMouse
	{
		HMouse();
		...
		// Check mouse state
		bool IsMouseButtonDown(HMouseButton bt) const;
		bool IsMouseButtonUp(HMouseButton bt) const;
		bool IsMouseButtonHeld(HMouseButton bt) const;

		HMouseRawDelta GetRawDelta() const noexcept;
		HMousePos GetMousePosition() const noexcept;

		HRelativeMouseMode GetRelativeMode() const noexcept;

		void FrameUpdate();
		void FixedUpdate();

		// Retrieve the current context of the mouse
		const HMouseContext& GetContext() const noexcept;
