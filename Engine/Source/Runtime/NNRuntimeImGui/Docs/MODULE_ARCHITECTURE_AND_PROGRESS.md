# NNRuntimeImGui — ImGui 与编辑器向 UI 扩展

> 曾用名：**VGImgui**；CMake 目标 **`NevernessRuntime-ImGui`**。
...
| **不负责** | 不负责游戏内 RmlUi 界面（见 **NNRuntimeRmlui**）；不负责主渲染帧图（见 **NNEngineLegacy**）。 |
| **CMake 目标** | `NevernessRuntime-ImGui`（`SHARED`） |
| **依赖** | `Freetype::Freetype`、`SDL3::SDL3`、`NevernessCore-Core`、`NevernessCore-PlatformCore`；平台 **OpenGL**（PRIVATE）。 |
| **宏** | `IMGUI_EXPORT`（`add_definitions`） |
| **典型消费者** | **NevernessRuntime-Core**、**NNEditor** 相关目标。 |

---

## 2. 构建与选项

- 使用全局 `include_directories(Runtime 根 + Core 根 + 本模块 Include)`（见 CMake）；新目标建议逐步迁移为 `target_include_directories` 风格（工程卫生项，非本文件范围）。
...
```
Engine/Source/Runtime/NNRuntimeImGui/
├── CMakeLists.txt
├── Docs/
...
需链接 **`NevernessRuntime-ImGui`** 并确保 **OpenGL + SDL3** 初始化顺序与 ImGui 后端一致（`imgui_impl_sdl3.h`、`imgui_impl_opengl3.h`）。
