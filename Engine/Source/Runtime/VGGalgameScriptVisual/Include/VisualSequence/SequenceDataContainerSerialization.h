...
#include "SequenceDataContainer.h"
...
namespace VisionGal
{
	 ...
	 */
	struct VG_GALGAME_VISUAL_SCRIPT_API IVGSSequenceComponentJsonBinding
	{
	 ...
	 */
	struct VG_GALGAME_VISUAL_SCRIPT_API VGSSequenceComponentJsonRegistry
	{
	...
	VG_GALGAME_VISUAL_SCRIPT_API nlohmann::json SerializeVGSSequenceDataContainerToJson(
	 ...
	 */
	VG_GALGAME_VISUAL_SCRIPT_API bool DeserializeVGSSequenceDataContainerFromJson(
		const nlohmann::json& root,
		VGSSequenceDataContainer& out);

	/// 便捷：序列化为字符串（indent < 0 表示紧凑输出）
	VG_GALGAME_VISUAL_SCRIPT_API std::string SerializeVGSSequenceDataContainerToString(
		const VGSSequenceDataContainer& container,
		int indent = 0);

	/// 便捷：从字符串解析；解析失败或内容校验失败返回 false
	VG_GALGAME_VISUAL_SCRIPT_API bool DeserializeVGSSequenceDataContainerFromString(
