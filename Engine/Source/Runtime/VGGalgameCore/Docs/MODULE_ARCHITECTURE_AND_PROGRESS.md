...
| **典型消费方** | **`VGGalgame`**（具体子系统实现）、**`VGGalgameRuntime`**（`StoryScriptSystem` 等）、**`VGGalgameScriptSequence`**、工具与测试工程。 |
...
flowchart TB
  ...
  subgraph Host["宿主实现 VGGalgame / VGGalgameRuntime"]
...
- **`IStoryScriptSystem`** 与 **`IStoryExecutionInstance`** 抽象，与 **`VGGalgameRuntime::StoryScriptSystem`**、**`VGGalgameScriptSequence`** 等协作。
...
- **`StoryScriptSystem`** 具体类在 **`VGGalgameRuntime`**，不在本库。
...
- **`IStoryScriptExecutor`**：契约位于 Core [`Interface/IStoryScriptExecutor.h`](../Interface/IStoryScriptExecutor.h)；Runtime [`IStoryScript.h`](../Interface/IStoryScript.h) 保留 **`GalGameScriptExecutorFactory`** 并 include Core 头。
...
| 日期 | 说明 |
|------|------|
| 2026-05-12 | Phase 7：`IStoryScriptExecutor` 入 Core；`SaveArchive` schema + `ValidateArchiveSchema`；Lua 子系统门面；`GalGameContext::Create`；`GalGameRuntimeState` 嵌套；Sequence 解链 Runtime；`GalGameSequenceScriptModuleMount` 迁至 `VGGalgame`。 |
