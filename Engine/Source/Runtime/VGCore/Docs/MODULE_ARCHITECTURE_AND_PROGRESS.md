...
| 项目 | 说明 |
|------|------|
| **职责** | 进程级 **应用壳**：窗口/视口、事件总线、输入、VFS 桥、Lua 辅助、与 **VGLua**/**VGRHI**/**VGImgui**/**VGPackage**/**HFileSystem**/**HCorePlatform** 等子系统绑定；对外暴露 **`Interface/`** 引擎接口与 **`Include/`** 内部子模块头。 |
| **不负责** | 不负责完整游戏循环与场景编辑（见 **VGEngine**）；不负责 RmlUi 文档模型（见 **VGUI**）。 |
| **CMake 目标** | `VGCore`（`SHARED`） |
| **宏** | `VGCORE_DYNAMIC`、`VG_CORE_API_EXPORT` |
| **依赖** | `VGLua`、`SDL3`、`SDL3_image`、`HCore`、`HCorePlatform`、`HFileSystem`、`VGImgui`、`VGRHI`、`VGPackage`（均为 PRIVATE，见 CMake）。 |
| **典型消费者** | **VGEngine**、编辑器模块、应用启动器。 |
