...
namespace NN::Runtime
{
	...
	void SceneLegacy::Update()
	{
	}
	 

	NN::Core::HECS* SceneLegacy::GetWorld()
	{
		return &m_Registry;
	}

	IGameActor* SceneLegacy::GetSceneActor()
	{
		return m_SceneActor.get();
	}


	VGActorID SceneLegacy::NewEntityID()
	{
		srand(std::clock());
	...
	VGComponentID SceneLegacy::NewComponentID()
	{
		srand(std::clock());
	...
	void SceneLegacy::SetEntityBaseData(IEntity* entity, VGActorID id, VGActorID parentID, const std::string& lable)
	{
	...
	IGameActor* SceneLegacy::CreateDeserializeActor(const SceneDeserializeEntity& deEntity)
	{
	...
	void SceneLegacy::UpdateDeserializeComponent(IEntity* entity, IComponent* component)
	{
		m_EntityComponentID.insert(component->EntityComID);
		component->SetOwner(entity);
	}

	void SceneLegacy::UpdateDeserializeActorRelationship()
	{
	...
	IGameActor* SceneLegacy::CreateActor(IEntity* parent)
	{
		auto entity = MakeRef<GameActor>();
		entity->SetBaseEntity(this->GetWorld()->create());
	...
	void SceneLegacy::CreateSceneActor()
	{
	...
	std::string SceneLegacy::GetNewLabel(IEntity* hObj)
	{
		return std::string("New Label");
	}

	SceneLegacy::SceneLegacy()
	{
		CreateSceneActor();
	}

	void SceneLegacy::AddEntityComponent(IEntity* entity, IComponent* component)
	{
	...
	IEntity* SceneLegacy::GetActor(VGActorID entityID)
	{
	...
	bool SceneLegacy::RemoveActor(VGActorID entityID)
	{
		if (auto result = m_IDEntityMap.find(entityID); result != m_IDEntityMap.end())
		{
	...
	bool SceneLegacy::ExistActor(VGActorID entityID)
	{
