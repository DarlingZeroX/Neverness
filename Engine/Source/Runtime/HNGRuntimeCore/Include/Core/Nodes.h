...
namespace Horizon::NodeGraphRuntime
{
	...
	H_NODE_GRAPH_API ExecResult SetVariableNodeExecute(RuntimeContext& ctx, NODE_ID nodeIndex);

	// GetVariable 节点执行函数：
	// - 从 ctx.variables[name] 中读取变量值，写入本节点的 "Value" 输出槽
	// - 便于后续节点通过普通数据槽访问变量
	H_NODE_GRAPH_API ExecResult GetVariableNodeExecute(RuntimeContext& ctx, NODE_ID nodeIndex);

	// Condition 节点执行函数（升级版 Branch）：
	// - 从配置的条件表达式字符串中求值（结果为 bool）
	// - 若为 true：激活 "True" 控制流输出；否则激活 "False" 输出
	// - 典型用法：condition="hp > 10 && hasKey == true"
	H_NODE_GRAPH_API ExecResult ConditionNodeExecute(RuntimeContext& ctx, NODE_ID nodeIndex);
