...
namespace Horizon::NodeGraphEditor
{
	...
	{
		// 第一次 Execute 时才生成缺失的 id
		// - Undo 不回收 id
		// - Redo 复用旧 id，避免链接/撤销历史错乱
		if (!m_Executed)
		{
			if (m_Node.id.Get() == 0)
				m_Node.id = m_Graph.idGen.NewNodeId();

			for (auto& p : m_Node.inputs)
				if (p.id.Get() == 0)
					p.id = m_Graph.idGen.NewPinId();

			for (auto& p : m_Node.outputs)
				if (p.id.Get() == 0)
					p.id = m_Graph.idGen.NewPinId();
		}
	...
	{
		// 第一次 Execute 时若 link id 未设置，则使用图内统一生成器
		// 注意：m_Executed 保证 Undo/Redo 不会再生成新 id
		if (!m_Executed && m_Link.id.Get() == 0)
			m_Link.id = m_Graph.idGen.NewLinkId();

		// 若 link 已存在则不重复插入
		auto it = std::find_if(m_Graph.links.begin(), m_Graph.links.end(),
			[&](const EditorLink& l)
			{
				return l.id == m_Link.id;
			});
		if (it != m_Graph.links.end())
			return;
