...
扩展 SequenceEditFieldId / EditSequencePropertyCommand（参考 `BuiltinSequenceInspectors.cpp` 中 `CommonDialogueSequenceInspector` 的实现）。
...
```text
Widget ──直接遍历──► document->GetSequence()->m_Sequence  // 禁止作为列表/时间轴数据源
```
...
- **禁止**：`for (auto& entry : document->GetSequence()->m_Sequence)` 驱动列表、时间轴、Outliner。
