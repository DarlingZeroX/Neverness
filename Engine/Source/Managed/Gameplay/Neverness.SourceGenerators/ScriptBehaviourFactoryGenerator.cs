// ============================================================================
// ScriptBehaviourFactoryGenerator.cs - 脚本行为工厂 Source Generator
// ============================================================================
// 扫描标记了 [AutoRegisterScript] 的类，生成 ScriptBehaviourFactory 静态工厂代码。
// ============================================================================
...
/// <summary>
/// 脚本行为工厂 Source Generator。
/// </summary>
[Generator]
public class ScriptBehaviourFactoryGenerator : IIncrementalGenerator
{
    // ========================================================================
    // 常量
    // ========================================================================
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // 3. 生成代码
        context.RegisterSourceOutput(collectedScripts, GenerateScriptBehaviourFactory);
    }
    ...
    /// <summary>生成脚本行为工厂代码。</summary>
    private static void GenerateScriptBehaviourFactory(
        ...
        ImmutableArray<ScriptClassInfo> scripts)
    {
        ...
        context.AddSource("ScriptBehaviourFactory_AutoGenerated.g.cs", SourceText.From(source, Encoding.UTF8));
    }
    ...
    private static string GenerateSource(ImmutableArray<ScriptClassInfo> scripts)
    {
        ...
        sb.AppendLine("    /// 脚本行为工厂自动生成代码。");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    public static class ScriptBehaviourFactory_AutoGenerated");
        sb.AppendLine("    {");

        // Create(Type, Entity) 方法
        sb.AppendLine("        /// <summary>创建 Behaviour 实例。</summary>");
        sb.AppendLine("        /// <param name=\"type\">脚本类型。</param>");
        sb.AppendLine("        /// <param name=\"entity\">所属实体。</param>");
        sb.AppendLine("        /// <returns>Behaviour 实例。</returns>");
        sb.AppendLine("        public static EntityBehaviour Create(Type type, Entity entity)");
        sb.AppendLine("        {");

        foreach (var script in scripts)
        {
            sb.AppendLine($"            if (type == typeof({script.FullName}))");
            sb.AppendLine("            {");
            sb.AppendLine($"                return new {script.FullName}() {{ Entity = entity }};");
            sb.AppendLine("            }");
        }

        sb.AppendLine();
        sb.AppendLine("            throw new ArgumentException($\"Unknown script type: {type.FullName}\");");
        sb.AppendLine("        }");
        sb.AppendLine();

        // Create<T>(Entity) 泛型方法
        sb.AppendLine("        /// <summary>创建 Behaviour 实例（泛型版本）。</summary>");
        sb.AppendLine("        /// <typeparam name=\"T\">脚本类型。</typeparam>");
        sb.AppendLine("        /// <param name=\"entity\">所属实体。</param>");
        sb.AppendLine("        /// <returns>Behaviour 实例。</returns>");
        sb.AppendLine("        public static T Create<T>(Entity entity) where T : EntityBehaviour, new()");
        sb.AppendLine("        {");
        sb.AppendLine("            return new T() { Entity = entity };");
        sb.AppendLine("        }");
        sb.AppendLine();

        // CreateInstance(Type) 方法（无 Entity 版本）
        sb.AppendLine("        /// <summary>创建 Behaviour 实例（无 Entity 版本）。</summary>");
        sb.AppendLine("        /// <param name=\"type\">脚本类型。</param>");
        sb.AppendLine("        /// <returns>Behaviour 实例。</returns>");
        sb.AppendLine("        public static EntityBehaviour CreateInstance(Type type)");
        sb.AppendLine("        {");
        ...
        foreach (var script in scripts)
        {
            sb.AppendLine($"            if (type == typeof({script.FullName}))");
            sb.AppendLine("            {");
            sb.AppendLine($"                return new {script.FullName}();");
            sb.AppendLine("            }");
        }
