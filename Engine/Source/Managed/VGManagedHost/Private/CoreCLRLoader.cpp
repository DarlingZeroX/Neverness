	...
	const std::filesystem::path* assembly_path_hint)
{
	...
	// Secondary hostfxr_initialize_for_runtime_config calls in the same process require a non-null
	// parameters pointer (host_path helps the muxer distinguish the native host from dotnet.exe).
	static thread_local std::wstring host_path_storage;
	wchar_t module_path[MAX_PATH]{};
	if (::GetModuleFileNameW(nullptr, module_path, MAX_PATH) != 0u)
	{
		host_path_storage.assign(module_path);
		init_params.host_path = host_path_storage.c_str();
	}
	else
	{
		host_path_storage.clear();
		init_params.host_path = nullptr;
	}
	init_params.dotnet_root = nullptr;
	hostfxr_handle ctx{};
	const int32_t rc = init(cfgW.c_str(), &init_params, &ctx);
	if ((rc != 0) && (rc != 1) && (rc != 2))
	{
		free_native_library(lib);
		return false;
	...
	hostfxr_initialize_parameters init_params{};
	init_params.size = sizeof(init_params);
	static thread_local std::string host_path_storage;
#if defined(__linux__)
	{
		constexpr std::size_t kBuf = 4096;
		host_path_storage.resize(kBuf);
		const ssize_t n = ::readlink("/proc/self/exe", host_path_storage.data(), kBuf);
		if (n > 0 && static_cast<std::size_t>(n) < kBuf)
		{
			host_path_storage.resize(static_cast<std::size_t>(n));
			init_params.host_path = host_path_storage.c_str();
		}
		else
		{
	...
	init_params.dotnet_root = nullptr;
	hostfxr_handle ctx{};
	const int32_t rc = init(cfg.c_str(), &init_params, &ctx);
	if ((rc != 0) && (rc != 1) && (rc != 2))
	{
		free_native_library(lib);
		return false;
