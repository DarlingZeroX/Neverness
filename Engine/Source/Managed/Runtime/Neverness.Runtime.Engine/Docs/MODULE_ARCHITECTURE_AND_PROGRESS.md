...
| **不負責** | CoreCLR 宿主、**`NNNativeAPI`** 宿主級欄位定義（見 **VisionGal.Managed.Core**）。 |
| **程式集** | **`VisionGal.Managed.Engine`**（`net10.0`，`AllowUnsafeBlocks`）、**`VisionGal.Managed.Engine.Runtime`**（薄封裝） |
| **依賴** | **VisionGal.Managed.Core**（取得 `NNNativeApi` / `NNNativeApiConstants`）。**Engine.Runtime** 另依賴 **VisionGal.Managed.Engine**。 |
...
| `EngineNativeApiBootstrap.InstallFromNativeApiTable` | 由 `NNNativeAPI*` 解析 `engineServices` 並按值快取函數指標。 |
...
| **2026-05-18** | **命名统一**：镜像类型 **`VG*` → `NN*`**；源文件 **`NNNativeEngineApiConstants.cs`** 等；**`EntityServiceAbiToken`** 与 Native **`NN_ENTITY_SERVICE_ABI_TOKEN`**（「NNEn」）对齐。 |
