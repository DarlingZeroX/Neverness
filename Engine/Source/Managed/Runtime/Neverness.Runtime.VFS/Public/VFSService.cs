// Neverness.Runtime.VFSService — 业务层 VFSService 门面（经函数表调用 Native VFSService）。
...
/// 调用前须完成 Native Bootstrap 且 VFSService 已由宿主挂载（例如 Editor <c>ApplicationHost.Initialize</c> 路径）。
/// </summary>
public static class VFSService
{
	/// <summary>VFSService 子表是否可用。</summary>
	public static bool IsAvailable => VFSHost.IsAvailable;

	/// <summary>
	/// 读取 VFSService 路径下的 UTF-8 文本全文；失败返回 <see langword="null"/>。
	/// </summary>
	/// <param name="path">VFSService 虚拟路径（将编码为 UTF-8）。</param>
	public static string? ReadText(string path) => VFSHost.ReadText(path);

	/// <summary>
	/// 将 UTF-8 文本写入 VFSService 路径（覆盖写）。
	/// </summary>
	public static bool WriteText(string path, string text) => VFSHost.WriteText(path, text);

	/// <summary>
	/// 读取 VFSService 路径下的二进制全文；失败返回 <see langword="null"/>；空文件返回零长度数组。
	/// </summary>
	...
	/// <summary>
	/// 计算 VFSService 相对路径（<c>VFSService::GetRelativePathVFS</c>）。
	/// </summary>
	...
	/// </summary>
	/// <param name="alias">VFSService 别名（如 <c>"/"</c>）。</param>
	...
	/// <summary>
	/// 查询 handle 对应的文件系统是否仍在 VFSService 中。
	/// </summary>
