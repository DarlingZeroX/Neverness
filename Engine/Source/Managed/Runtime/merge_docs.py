...
def merge_docs():
    ...
    runtime_engine_abi = (
        current_dir.parent / "Runtime" / "NNNativeEngineAPI" / "Docs" / "MODULE_ARCHITECTURE_AND_PROGRESS.md"
    )
    runtime_stub = (
        current_dir.parent
        / "Runtime"
    ...
    vge_rt = current_dir.parent / "Runtime" / "NNRuntimeEngine" / "Docs" / "MODULE_ARCHITECTURE_AND_PROGRESS.md"
    vge_rt_svc = current_dir.parent / "Runtime" / "NNRuntimeEngineServices" / "Docs" / "MODULE_ARCHITECTURE_AND_PROGRESS.md"
    extra_runtime_docs = []
    for p in (runtime_engine_abi, runtime_stub, vge_rt, vge_rt_svc):
        if p.is_file():
    ...
    with open(output_file, "w", encoding="utf-8") as outfile:
        outfile.write(
            ...
            "以及（可選）`Engine/Source/Runtime/NNNativeEngineAPI/Docs/...`、`NNRuntimeNativeEngineAPIStub/Docs/...`、"
            "`NNRuntimeEngine/Docs/...`、`NNRuntimeEngineServices/Docs/...` 後重新執行本腳本。\n\n"
