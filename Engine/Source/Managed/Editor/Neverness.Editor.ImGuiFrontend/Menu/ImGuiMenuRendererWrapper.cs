...
/// <summary>
/// ImGui 菜单渲染器——实现 IMenuRenderer 接口。
/// 递归遍历 MenuTree 绘制主菜单栏。
///
/// 从 Framework/Private/Menu/ImGuiMenuRenderer.cs 搬迁而来。
/// </summary>
public sealed class ImGuiMenuRendererWrapper : IMenuRenderer
{
    /// <summary>全局静态实例——供旧代码以静态方式调用。</summary>
    public static ImGuiMenuRendererWrapper Instance { get; } = new();

    /// <summary>渲染主菜单栏（完整生命周期：Begin → 菜单项 → End）。</summary>
    public void RenderMainMenuBar(MenuTree tree)
    {
    ...
    public void RenderMenuItems(MenuTree tree)
    {
