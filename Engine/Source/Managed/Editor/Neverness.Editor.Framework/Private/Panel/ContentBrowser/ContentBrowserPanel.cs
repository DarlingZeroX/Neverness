...
public class ContentBrowserPanel : IEditorPanel
{
    ...
    /// 外部可通过 <see cref="Public.ContentBrowserContextMenu"/> 扩展。
    /// </summary>
    private void ShowBackgroundContextMenu(string path)
    {
        ...
        // 注入运行时上下文，供贡献者命令回调读取
        ctx.SetContext(Public.ContentBrowserContextMenu.KeyPath, path);
        ctx.SetContext(Public.ContentBrowserContextMenu.KeyContentBrowser, _contentBrowser);

        ctx.RenderWindowContextMenu(Public.ContentBrowserContextMenu.BackgroundId);
    }
    ...
    private void ShowItemContextMenu(ContentItem item)
    {
        ...
        // 注入运行时上下文，供贡献者命令回调读取
        ctx.SetContext(Public.ContentBrowserContextMenu.KeyItem, item);
        ctx.SetContext(Public.ContentBrowserContextMenu.KeyContentBrowser, _contentBrowser);

        ctx.RenderItemContextMenu(Public.ContentBrowserContextMenu.ItemId);
    }
