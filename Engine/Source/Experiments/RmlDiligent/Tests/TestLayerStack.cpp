// =============================================================================
// TestLayerStack.cpp
// Phase 5a: LayerStack + RT Pool 验证
// =============================================================================
    ...
    Diligent::ISwapChain** ppSwapChain)
{
    ...
    if (auto* factory = Diligent::GetEngineFactoryD3D12()) {
        ...
        if (*ppDevice) {
            ...
            if (*ppSwapChain) {
                std::cout << "[Phase5a] Backend: D3D12" << std::endl;
                return true;
            }
        }
    }
    ...
    if (auto* factory = Diligent::GetEngineFactoryD3D11()) {
        ...
        if (*ppDevice) {
            ...
            if (*ppSwapChain) {
                std::cout << "[Phase5a] Backend: D3D11" << std::endl;
                return true;
            }
        }
    }
...
<rml><head><title>Phase 5a Inline</title></head><body style="display: block; margin: 0; width: 1024dp; height: 768dp; background: #1a1a2e;">
<div id="opacity-panel" style="display: block; position: absolute; left: 40dp; top: 40dp; width: 220dp; height: 140dp; padding: 12dp; background: #3366cc; color: #ffffff; font-size: 16dp; font-family: LatoLatin; filter: opacity(0.55);">opacity</div>
<div id="shadow-panel" style="display: block; position: absolute; left: 300dp; top: 40dp; width: 220dp; height: 140dp; padding: 12dp; background: #cc6633; color: #ffffff; font-size: 16dp; font-family: LatoLatin; box-shadow: 0dp 8dp 16dp #00000088;">shadow</div>
<p id="ref" style="display: block; position: absolute; left: 40dp; top: 240dp; width: 180dp; height: 80dp; background: #444444; color: #cccccc; font-size: 14dp; font-family: LatoLatin;">ref</p>
</body></rml>
)";

static const char* kInlineOpacityRml = R"(
<rml><head><title>Opacity</title></head><body style="display: block; margin: 0; width: 1024dp; height: 768dp; background: #1a1a2e;">
<div id="opacity-panel" style="display: block; position: absolute; left: 40dp; top: 40dp; width: 220dp; height: 140dp; padding: 12dp; background: #3366cc; color: #ffffff; font-size: 16dp; font-family: LatoLatin; filter: opacity(0.55);">opacity</div>
<p id="ref" style="display: block; position: absolute; left: 40dp; top: 240dp; width: 180dp; height: 80dp; background: #444444; color: #cccccc; font-size: 14dp; font-family: LatoLatin;">ref</p>
</body></rml>
)";

static const char* kInlineShadowRml = R"(
<rml><head><title>Shadow</title></head><body style="display: block; margin: 0; width: 1024dp; height: 768dp; background: #1a1a2e;">
<div id="shadow-panel" style="display: block; position: absolute; left: 40dp; top: 40dp; width: 220dp; height: 140dp; padding: 12dp; background: #cc6633; color: #ffffff; font-size: 16dp; font-family: LatoLatin; box-shadow: 0dp 8dp 16dp #00000088;">shadow</div>
<p id="ref" style="display: block; position: absolute; left: 40dp; top: 240dp; width: 180dp; height: 80dp; background: #444444; color: #cccccc; font-size: 14dp; font-family: LatoLatin;">ref</p>
</body></rml>
)";

static const char* InlineRmlForCase(LayerStackTestCase testCase)
{
    switch (testCase) {
        case LayerStackTestCase::Opacity:
            return kInlineOpacityRml;
        case LayerStackTestCase::Shadow:
            return kInlineShadowRml;
        case LayerStackTestCase::All:
        default:
            return kInlineAllRml;
    }
}

static std::vector<const char*> RequiredElementIds(LayerStackTestCase testCase)
{
    switch (testCase) {
        case LayerStackTestCase::Opacity:
        ...
        case LayerStackTestCase::All:
        default:
            return {"opacity-panel", "shadow-panel", "ref"};
    }
}

static Rml::ElementDocument* LoadLayerStackDocument(Rml::Context* rmlContext, LayerStackTestCase testCase)
{
    if (testCase == LayerStackTestCase::All) {
        if (Rml::ElementDocument* document = rmlContext->LoadDocument("phase5a_layers.rml")) {
            std::cout << "[OK] Loaded phase5a_layers.rml" << std::endl;
            return document;
        }
        std::cerr << "[WARN] phase5a_layers.rml failed, using inline RML" << std::endl;
    }

    Rml::ElementDocument* document = rmlContext->LoadDocumentFromMemory(InlineRmlForCase(testCase), "phase5a_layers.rml");
    if (document) {
        std::cout << "[OK] Inline layer stack document loaded" << std::endl;
    }
    return document;
}
...
static LayerStackTestCase ParseTestCase(const std::string& value)
{
    if (value == "opacity") {
        return LayerStackTestCase::Opacity;
    }
    if (value == "shadow") {
        return LayerStackTestCase::Shadow;
    }
    return LayerStackTestCase::All;
}

static const char* TestCaseName(LayerStackTestCase testCase)
{
    switch (testCase) {
        case LayerStackTestCase::Opacity:
            return "opacity";
...
int main(int argc, char* argv[])
{
    ...
    LayerStackTestCase testCase = LayerStackTestCase::All;
    ...
    std::cout << "============================================" << std::endl;
    std::cout << "  RmlDiligent Phase 5a: LayerStack + RT Pool" << std::endl;
    ...
    SDL_Window* window = SDL_CreateWindow("RmlDiligent Phase 5a", 1024, 768, SDL_WINDOW_RESIZABLE);
    if (!window) {
    ...
    Rml::ElementDocument* document = LoadLayerStackDocument(rmlContext, testCase);
    if (!document) {
    ...
    if (testCase == LayerStackTestCase::All || testCase == LayerStackTestCase::Opacity) {
        std::cout << "  - #opacity-panel: filter triggers PushLayer/Composite" << std::endl;
    }
    if (testCase == LayerStackTestCase::All || testCase == LayerStackTestCase::Shadow) {
        std::cout << "  - #shadow-panel: box-shadow triggers layer stack" << std::endl;
    }
    ...
    while (running) {
        ...
        if (frameCount == 0 || frameCount == 2) {
            std::cout << "[INFO] frame " << frameCount << " drawCount=" << lastRmlDrawCount
                      << " pushLayers=" << lastPushLayerCount << " composites=" << lastCompositeCount << std::endl;
        }
    ...
    std::cout << "\n============================================" << std::endl;
    if (elementsOk && renderOk && layerOk) {
        std::cout << "  [PASS] Phase 5a 通过!" << std::endl;
    } else {
        std::cout << "  [FAIL] Phase 5a 验收未通过" << std::endl;
        if (!elementsOk) {
    ...
    std::cout << "  last drawCount: " << lastRmlDrawCount << " pushLayers: " << lastPushLayerCount
              << " composites: " << lastCompositeCount << std::endl;
    std::cout << "============================================" << std::endl;
    ...
    return (elementsOk && renderOk && layerOk) ? 0 : 1;
}
