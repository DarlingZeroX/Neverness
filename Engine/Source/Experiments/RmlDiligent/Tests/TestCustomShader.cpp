// =============================================================================
// TestCustomShader.cpp
// Phase 3: Gradient + Creation 验证
// =============================================================================
    ...
    Diligent::ISwapChain** ppSwapChain)
{
    ...
    if (auto* factory = Diligent::GetEngineFactoryD3D12()) {
        ...
        if (*ppDevice) {
            ...
            if (*ppSwapChain) {
                std::cout << "[Phase3] Backend: D3D12" << std::endl;
                return true;
            }
        }
    }
    ...
    if (auto* factory = Diligent::GetEngineFactoryD3D11()) {
        ...
        if (*ppDevice) {
            ...
            if (*ppSwapChain) {
                std::cout << "[Phase3] Backend: D3D11" << std::endl;
                return true;
            }
        }
    }
...
// RmlUi 6.x：decorator 须在 <style> 中声明；内联 style 里的 decorator 不会解析为 DECORATOR 属性。
static const char* kInlineAllRml = R"(
<rml><head><title>Phase 3 Inline</title>
<style>
#linear { position: absolute; left: 40dp; top: 40dp; width: 200dp; height: 120dp; decorator: linear-gradient(90deg, #ff0000, #0000ff); }
#radial { position: absolute; left: 280dp; top: 40dp; width: 200dp; height: 120dp; decorator: radial-gradient(circle, #ffff00, #8800cc); }
#conic { position: absolute; left: 520dp; top: 40dp; width: 200dp; height: 120dp; decorator: conic-gradient(from 0deg, red, lime, blue); }
#creation { position: absolute; left: 40dp; top: 280dp; width: 240dp; height: 160dp; decorator: shader("creation"); }
</style></head><body>
<div id="linear"></div><div id="radial"></div><div id="conic"></div><div id="creation"></div>
</body></rml>
)";

static const char* kInlineLinearRml = R"(
<rml><head><title>Linear</title>
<style>
#linear { position: absolute; left: 40dp; top: 40dp; width: 200dp; height: 120dp; decorator: linear-gradient(90deg, #ff0000, #0000ff); }
</style></head><body><div id="linear"></div></body></rml>
)";

static const char* kInlineRadialRml = R"(
<rml><head><title>Radial</title>
<style>
#radial { position: absolute; left: 40dp; top: 40dp; width: 200dp; height: 120dp; decorator: radial-gradient(circle, #ffff00, #8800cc); }
</style></head><body><div id="radial"></div></body></rml>
)";

static const char* kInlineConicRml = R"(
<rml><head><title>Conic</title>
<style>
#conic { position: absolute; left: 40dp; top: 40dp; width: 200dp; height: 120dp; decorator: conic-gradient(from 0deg, red, lime, blue); }
</style></head><body><div id="conic"></div></body></rml>
)";

static const char* kInlineCreationRml = R"(
<rml><head><title>Creation</title>
<style>
#creation { position: absolute; left: 40dp; top: 40dp; width: 240dp; height: 160dp; decorator: shader("creation"); }
</style></head><body><div id="creation"></div></body></rml>
)";

static const char* InlineRmlForCase(ShaderTestCase testCase)
{
    switch (testCase) {
        case ShaderTestCase::Linear:
            return kInlineLinearRml;
        case ShaderTestCase::Radial:
            return kInlineRadialRml;
        case ShaderTestCase::Conic:
            return kInlineConicRml;
        case ShaderTestCase::Creation:
            return kInlineCreationRml;
        case ShaderTestCase::All:
        default:
            return kInlineAllRml;
    }
}

static std::vector<const char*> RequiredElementIds(ShaderTestCase testCase)
{
    switch (testCase) {
        case ShaderTestCase::Linear:
            return {"linear"};
        ...
        case ShaderTestCase::All:
        default:
            return {"linear", "radial", "conic", "creation"};
    }
}

static Rml::ElementDocument* LoadShaderDocument(Rml::Context* rmlContext, ShaderTestCase testCase)
{
    if (testCase == ShaderTestCase::All) {
        if (Rml::ElementDocument* document = rmlContext->LoadDocument("phase3_gradients.rml")) {
            std::cout << "[OK] Loaded phase3_gradients.rml" << std::endl;
            return document;
        }
        std::cerr << "[WARN] phase3_gradients.rml failed, using inline RML" << std::endl;
    }

    Rml::ElementDocument* document = rmlContext->LoadDocumentFromMemory(InlineRmlForCase(testCase), "phase3_gradients.rml");
    if (document) {
        std::cout << "[OK] Inline shader document loaded" << std::endl;
    }
    return document;
}
...
static ShaderTestCase ParseTestCase(const std::string& value)
{
    if (value == "linear") {
        return ShaderTestCase::Linear;
    }
    if (value == "radial") {
        return ShaderTestCase::Radial;
    }
    if (value == "conic") {
        return ShaderTestCase::Conic;
    }
    if (value == "creation") {
        return ShaderTestCase::Creation;
    }
    return ShaderTestCase::All;
}

static const char* TestCaseName(ShaderTestCase testCase)
{
    switch (testCase) {
        case ShaderTestCase::Linear:
            return "linear";
...
int main(int argc, char* argv[])
{
    ...
    ShaderTestCase testCase = ShaderTestCase::All;
    ...
    std::cout << "============================================" << std::endl;
    std::cout << "  RmlDiligent Phase 3: Gradient + Creation" << std::endl;
    ...
    SDL_Window* window = SDL_CreateWindow("RmlDiligent Phase 3", 1024, 768, SDL_WINDOW_RESIZABLE);
    if (!window) {
    ...
    Rml::ElementDocument* document = LoadShaderDocument(rmlContext, testCase);
    if (!document) {
    ...
    rmlContext->Update();
    rmlContext->Render();
    const uint32_t layoutCompileShaders = renderInterface.GetCompileShaderCount();
    ...
    for (const char* id : requiredIds) {
        ...
        std::cout << "[INFO] #" << id << " layout: " << size.x << " x " << size.y
                  ...
                  << " visible=" << (element->IsVisible(true) ? "yes" : "no")
                  << " has_decorator=" << (element->GetComputedValues().has_decorator() ? "yes" : "no") << std::endl;
        if (const Rml::Property* decoratorProp = element->GetProperty("decorator")) {
            std::cout << "[INFO] #" << id << " decorator unit=" << static_cast<int>(decoratorProp->unit) << std::endl;
        } else {
            std::cerr << "[WARN] #" << id << " has no decorator property" << std::endl;
        }
    ...
    std::cout << "[INFO] compileShaderCount after layout render: " << layoutCompileShaders << std::endl;

    const size_t minDrawCount = requiredIds.size();
    std::cout << "\n[INFO] Visual checklist:" << std::endl;
    if (testCase == ShaderTestCase::All || testCase == ShaderTestCase::Linear) {
        std::cout << "  - #linear: left red -> right blue horizontal gradient" << std::endl;
    }
    if (testCase == ShaderTestCase::All || testCase == ShaderTestCase::Radial) {
        std::cout << "  - #radial: yellow center, purple edges" << std::endl;
    }
    if (testCase == ShaderTestCase::All || testCase == ShaderTestCase::Conic) {
        std::cout << "  - #conic: red/lime/blue color wheel" << std::endl;
    }
    if (testCase == ShaderTestCase::All || testCase == ShaderTestCase::Creation) {
        std::cout << "  - #creation: animated procedural pattern (changes over time)" << std::endl;
    }
    ...
    uint32_t lastShaderDrawCount = 0;
    ...
    while (running) {
        ...
        lastShaderDrawCount = renderInterface.GetShaderDrawCount();

        if (frameCount == 0 || frameCount == 2) {
            std::cout << "[INFO] frame " << frameCount << " drawCount=" << lastRmlDrawCount
                      << " shaderDraws=" << lastShaderDrawCount << std::endl;
        }
    ...
    const bool shaderOk = lastShaderDrawCount > 0;

    std::cout << "\n============================================" << std::endl;
    if (elementsOk && renderOk && shaderOk) {
        std::cout << "  [PASS] Phase 3 通过!" << std::endl;
    } else {
        std::cout << "  [FAIL] Phase 3 验收未通过" << std::endl;
        if (!elementsOk) {
        ...
        if (!shaderOk) {
            std::cout << "  - shaderDraws=0（Gradient/Creation 未渲染）" << std::endl;
        }
    }
    std::cout << "  case: " << TestCaseName(testCase) << std::endl;
    ...
    std::cout << "  last drawCount: " << lastRmlDrawCount << " shaderDraws: " << lastShaderDrawCount << std::endl;
    std::cout << "============================================" << std::endl;
    ...
    return (elementsOk && renderOk && shaderOk) ? 0 : 1;
}
