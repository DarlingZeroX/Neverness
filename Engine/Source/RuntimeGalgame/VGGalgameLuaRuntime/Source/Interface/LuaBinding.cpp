...
namespace VisionGal::GalGame
{
	...
	void GalGameLuaBinding::Register(sol::state& state)
	{
		...
		galgame.new_usertype<IGalRuntimeSession>("IGalRuntimeSession",
			"Tick", &IGalRuntimeSession::Tick,
			"SubsystemBus", sol::property(
				[](IGalRuntimeSession& self) -> ISubsystemBus* { return self.GetSubsystemBus(); }
			),
			"Scheduler", sol::property(
				[](IGalRuntimeSession& self) -> IExecutionScheduler* { return self.GetExecutionScheduler(); }
			)
		);
