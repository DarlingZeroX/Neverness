...
public static unsafe class NativeHandleBridge
{
	...
	public static VGObjectHandle CreateObject(ReadOnlySpan<byte> typeNameUtf8)
	{
		...
		fixed (byte* p = typeNameUtf8)
		{
			...
			return new VGObjectHandle(h);
		}
	}

	/// <summary>以 managed 字串建立 Native 物件控制代碼。</summary>
	public static VGObjectHandle CreateObject(string typeName)
	{
	...
	public static void DestroyObject(VGObjectHandle handle)
	{
	...
	public static void Retain(VGObjectHandle handle)
	{
		if (handle.Value == 0 || !EngineNativeApiBootstrap.IsInstalled)
		{
			return;
	...
	public static void Release(VGObjectHandle handle)
	{
		if (handle.Value == 0 || !EngineNativeApiBootstrap.IsInstalled)
		{
			return;
	...
	public static uint GetRefCount(VGObjectHandle handle)
	{
		if (handle.Value == 0 || !EngineNativeApiBootstrap.IsInstalled)
		{
			return 0;
	...
	public static bool IsAlive(VGObjectHandle handle)
	{
		if (handle.Value == 0 || !EngineNativeApiBootstrap.IsInstalled)
		{
			return false;
	...
	public static string? TryGetTypeName(VGObjectHandle handle)
	{
		if (handle.Value == 0 || !EngineNativeApiBootstrap.IsInstalled)
		{
			return null;
